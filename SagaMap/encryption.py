def GenerateDecExpKey(key):
    ...
    expandedKey = [0] * (len(exKey) << 2)
    j = -1

    for i in reversed(range(len(exKey))):
        expandedKey[j+1] = (exKey[i] & 0xff)
        expandedKey[j+2] = ((exKey[i] >> 8) & 0xff)
        expandedKey[j+3] = ((exKey[i] >> 16) & 0xff)
        expandedKey[j+4] = (exKey[i] >> 24)
        j+=4

    return bytearray(expandedKey[:16])
